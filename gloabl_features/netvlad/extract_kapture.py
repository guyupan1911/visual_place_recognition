...
def extract_kapture_global_features(kapture_root_path: str, global_features_type: str):
    ...
    with get_all_tar_handlers(kapture_root_path,
        ...
        input_transform = netvlad.input_transform()
        ...
        for image_name in tqdm.tqdm(imgs):
            ...
            img = input_transform(img).unsqueeze(0).to('cuda')
            global_feature = net.pool(net.encoder(img)).squeeze(0).detach().cpu().numpy().astype(np.float32)
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Extract NetVLAD global features')
    parser.add_argument('--kapture-root', type=str, required=True, help='path to kapture root directory')
    args = parser.parse_args()

    # Evaluate
    res = extract_kapture_global_features(args.kapture_root, "NetVLAD")
