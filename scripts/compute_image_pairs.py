...
def specified_camera_id(camera_id: str, image_list: list):
    ret = []
    for image_name, gfeat_path in image_list:
        if image_name.split('/')[0] == camera_id:
            ret.append((image_name, gfeat_path))
    return ret
...
def compute_image_pairs(mapping_path: str,
    ...
    if mapping_path == query_path:
        ...
        with get_all_tar_handlers(query_path) as query_tar_handlers:
            ...
            query_list = specified_camera_id(camera_id,
                list(sorted(query_global_features_to_filepaths.items())))
...
def compute_image_pairs_command_line():
    ...
    parser.add_argument('--camera-id', default='right_0_n_6mm', help='camera identifier')
    ...
    compute_image_pairs(args.mapping, args.query, args.output, args.global_features_type, args.topk, args.camera_id)
